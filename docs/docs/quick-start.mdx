...
CatDb is a .NET library. The current documented release is **v<CurrentVersion />**.

Add the package with the same version:

<PackageInstall />

CatDb is **server-first**: point client code at a running `CatDb.Server`/TCP server and open tables the same way you would locally. Embedding the engine directly in your process (no server) is the other supported mode, for single-process apps, tests, and demos.

Once you have an engine — remote or embedded — open a table, write records, and call `Commit()` when those writes must survive process exit.
...
Connect to a server (see [Setup server](setup-server.md) to run one):
...
Or use one connection string for any backend — `FromConnectionString` picks File/Memory/Network from the string itself (see [Database engine](database-engine.md#connection-strings) for the full key reference):
...
For embedded use (no server process), open a file, an in-memory store, or any stream directly:
