...
public class Scheme : IEnumerable<KeyValuePair<long, Locator>>
{
    ...
    private long ObtainPathId() => Interlocked.Increment(ref _locatorId);
    ...
    public static Scheme Deserialize(BinaryReader reader)
    {
        ...
        var scheme = new Scheme { _locatorId = reader.ReadInt64() };
    ...
    public IEnumerator<KeyValuePair<long, Locator>> GetEnumerator() =>
        _map.Select(s => new KeyValuePair<long, Locator>(s.Key, s.Value)).GetEnumerator();
