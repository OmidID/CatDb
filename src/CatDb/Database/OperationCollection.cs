...
public class OperationCollection : List<IOperation>, IOperationCollection
{
    ...
    public OperationCollection(Locator locator, int capacity) : base(capacity)
    {
        Locator                = locator;
    ...
    public new void Add(IOperation operation)
    {
        if (AreAllMonotoneAndPoint)
        {
            ...
            else if (operation.Scope != OperationScope.Point || Locator.KeyComparer.Compare(operation.FromKey, this[Count - 1].FromKey) <= 0)
    ...
    public void AddRange(IOperationCollection operations)
    {
        if (!operations.AreAllMonotoneAndPoint)
            AreAllMonotoneAndPoint = false;
        else if (AreAllMonotoneAndPoint && Count > 0 && operations.Count > 0 &&
                 Locator.KeyComparer.Compare(this[Count - 1].FromKey, operations[0].FromKey) >= 0)
            AreAllMonotoneAndPoint = false;
